package com.ntmsecurity.sova.logic;
...
import com.ntmsecurity.sova.ui.DummyCameraActivity;
import com.ntmsecurity.sova.ui.LockScreenMessage;
import com.ntmsecurity.sova.R;
import com.ntmsecurity.sova.utils.CypherUtils;
import com.ntmsecurity.sova.logic.command.helper.GPS;
import com.ntmsecurity.sova.utils.Logger;
import com.ntmsecurity.sova.logic.command.helper.Network;
...
public class MessageHandler {
    ...
    public String handle(String msg, Context context) {
        ...
        if(msg.startsWith((String) ch.getSettings().get(Settings.SET_Sova_COMMAND))) {
            int cutLength = ((String) ch.getSettings().get(Settings.SET_Sova_COMMAND)).length();
            ...
            //DELETE
                ...
                if ((Boolean) ch.getSettings().get(Settings.SET_WIPE_ENABLED)) {
                    ...
                    if (msg.length() > COM_DELETE.length()+1) {
                        ...
                        replyBuilder.append(context.getString(R.string.MH_Syntax)).append((String) ch.getSettings().get(Settings.SET_Sova_COMMAND)).append(" delete [pin]");
                    }
                }
            ...
            //EXPERT
                ...
                executedCommand = COM_DELETE;
                replyBuilder.append((String) ch.getSettings().get(Settings.SET_Sova_COMMAND)).append(context.getString(R.string.MH_Help_Expert_GPS)).append("\n");
                replyBuilder.append((String) ch.getSettings().get(Settings.SET_Sova_COMMAND)).append(context.getString(R.string.MH_Help_Expert_Sound)).append("\n");
                replyBuilder.append((String) ch.getSettings().get(Settings.SET_Sova_COMMAND)).append(context.getString(R.string.MH_Help_Expert_Camera)).append("\n");
                ...
                if(Permission.CAMERA) {
                    ...
                    replyBuilder.append(context.getString(R.string.MH_CAM_CAPTURE)).append((String) ch.getSettings().get(Settings.SET_SovaSERVER_URL));
                }
            }else{
                replyBuilder.append(context.getString(R.string.MH_Title_Help)).append("\n");
                if (Permission.GPS) {
                    replyBuilder.append((String) ch.getSettings().get(Settings.SET_Sova_COMMAND)).append(context.getString(R.string.MH_Help_where)).append("\n");
                }
                replyBuilder.append((String) ch.getSettings().get(Settings.SET_Sova_COMMAND)).append(context.getString(R.string.MH_Help_ring)).append("\n");
                if (Permission.DEVICE_ADMIN) {
                    replyBuilder.append((String) ch.getSettings().get(Settings.SET_Sova_COMMAND)).append(context.getString(R.string.MH_Help_Lock)).append("\n");
                }
                replyBuilder.append((String) ch.getSettings().get(Settings.SET_Sova_COMMAND)).append(context.getString(R.string.MH_Help_Stats));
                if ((Boolean) ch.getSettings().get(Settings.SET_WIPE_ENABLED)) {
                    replyBuilder.append("\n").append((String) ch.getSettings().get(Settings.SET_Sova_COMMAND)).append(context.getString(R.string.MH_Help_delete));
                }
                replyBuilder.append("\n").append((String) ch.getSettings().get(Settings.SET_Sova_COMMAND)).append(context.getString(R.string.MH_Help_expert));
            ...
            if (!reply.isEmpty() && !silent) {
                ...
                int counter = (Integer) ch.getSettings().get(Settings.SET_SovaSMS_COUNTER);
                counter++;
                ch.getSettings().set(Settings.SET_SovaSMS_COUNTER, counter);
    ...
    public boolean checkForPin(String msg) {
        if (msg.length() > ((String) ch.getSettings().get(Settings.SET_Sova_COMMAND)).length()+1) {
            String pin = msg.substring(((String) ch.getSettings().get(Settings.SET_Sova_COMMAND)).length() + 1);

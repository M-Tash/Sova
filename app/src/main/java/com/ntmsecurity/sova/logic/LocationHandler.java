...
public class LocationHandler {
    ...
    public void newLocation(String provider, String lat, String lon){
        ...
        if(sendToServer || (Boolean) ch.getSettings().get(Settings.SET_SovaSERVER_AUTO_UPLOAD)){
            String id =  (String) ch.getSettings().get(Settings.SET_SovaSERVER_ID);
            if(!id.isEmpty()) {
                SovaServerService.sendNewLocation(ch.getContext(), ch.getSettings(), provider, lat, lon);
            }
        }
    }

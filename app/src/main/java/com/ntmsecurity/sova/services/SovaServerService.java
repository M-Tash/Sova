...
import com.ntmsecurity.sova.net.RespHandler;
...
import com.ntmsecurity.sova.utils.PatchedVolley;
import com.ntmsecurity.sova.utils.Permission;

@SuppressLint("NewApi")
public class SovaServerService extends JobService {
    ...
    public static void registerOnServer(Context context, String url, String privKey, String pubKey, String hashedPW) {
        ...
        RespHandler respHandler = new RespHandler(response -> {
            try {
                settings.set(Settings.SET_SovaSERVER_ID, response.get("DeviceId"));
            } catch (JSONException e) {
                e.printStackTrace();
            }
        });
    ...
    public static void unregisterOnServer(Context context) {
        ...
        Settings settings = JSONFactory.convertJSONSettings(IO.read(JSONMap.class, IO.settingsFileName));
        RequestQueue queue = PatchedVolley.newRequestQueue(context);
        String url = (String)settings.get(Settings.SET_SovaSERVER_URL);
        final JSONObject requestAccessObject = new JSONObject();

        DataHandler dataHandler = new DataHandler(context);
        dataHandler.run(DataHandler.DEVICE,null);
        settings.set(Settings.SET_SovaSERVER_ID, "");
        settings.set(Settings.SET_SovaSERVER_AUTO_UPLOAD, false);
        settings.setNow(Settings.SET_SovaSERVER_UPLOAD_SERVICE, false);
    }

    public static void scheduleJob(Context context, int time) {
        ComponentName serviceComponent = new ComponentName(context, SovaServerService.class);
        ...
        Logger.logSession("SovaServerService", "scheduled new job");
    ...
    public boolean onStartJob(JobParameters params) {
        ...
        Logger.logSession("SovaServerService", "started");
        Settings settings = JSONFactory.convertJSONSettings(IO.read(JSONMap.class, IO.settingsFileName));
        if ((Boolean) settings.get(Settings.SET_SovaSERVER_UPLOAD_SERVICE)) {
            ...
            boolean registered = !((String) ch.getSettings().get(Settings.SET_SovaSERVER_ID)).isEmpty();
            if (registered) {
                ...
                switch ((Integer) ch.getSettings().get(Settings.SET_SovaSERVER_LOCATION_TYPE)) {
                    case 0:
                        locateCommand += " gps";
                        break;
                    case 1:
                ...
                ch.getMessageHandler().handle(ch.getSettings().get(Settings.SET_Sova_COMMAND) + locateCommand, this);
            }
            Logger.logSession("SovaServerService", "finished job, waiting for location");
            Logger.writeLog();
    ...
    public boolean onStopJob(JobParameters params) {
        Logger.log("SovaServerService", "job stopped by system");
        Settings settings = JSONFactory.convertJSONSettings(IO.read(JSONMap.class, IO.settingsFileName));
        scheduleJob(this, (Integer)settings.get(Settings.SET_SovaSERVER_UPDATE_TIME));
        return false;
    }
}

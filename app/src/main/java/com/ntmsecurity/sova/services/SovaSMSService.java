...
import com.ntmsecurity.sova.logic.MessageHandler;
...
public class SovaSMSService extends JobService {
    ...
    public static void scheduleJob(Context context, String destination, String message, Long time) {
        ...
        ComponentName serviceComponent = new ComponentName(context, SovaSMSService.class);
    ...
    public boolean onStartJob(JobParameters params) {
        ...
        for (int iwl = 0; iwl < whiteList.size(); iwl++) {
            if (PhoneNumberUtils.compare(whiteList.get(iwl).getNumber(), receiver)) {
                Logger.logSession("Usage", receiver + " used Sova");
